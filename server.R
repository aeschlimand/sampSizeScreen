...
library(shiny)
library(ggplot2)
# # Note that these next lines go together   
...
server <- function(input, output, session) {
  # source("auxFnsIndividualHistCompound1.R")
  ...
  output$howHeavyTails <- renderPlot({
    ...
    plot(seqq, ys, pch=16, xlim=c(-5, 5), ylim=c(0, 0.4), xlab='', ylab='', main='',
         yaxt='n', xaxt='n', col=colsNow[3])
    ys2=dnorm(seqq)
    points(seqq, ys2, pch=16, col=colsNow[8])
    points(seqq, ys, pch=16, col=colsNow[3])
    text(3, 0.3, "Normal", col=colsNow[8], cex=2)
  }, height=250, width=250)
  # Histograms for FOVs of compound classes as they are being chosen----
  # Graph for first class of compound ----
  output$class1pdf <- renderPlot({
    # Read in needed values
    SSMD.NowA=as.numeric(input$SSMD.t1)
    sd.NowA=input$sd.t1
    metricNowA=input$metricNow
    cat("metricNowA\n")
    print(metricNowA)
    dfPerFOVA=dfCloseToNormal - input$tailWeight
    mu.DMSOA=input$mu.DMSO
    sdDMSOA=input$sdDMSO
    getGraphCentered(SSMD.Now=SSMD.NowA, sd.Now=sd.NowA, metricNow=metricNowA,
      dfPerFOV=dfPerFOVA, mu.DMSO=mu.DMSOA, sdDMSO=sdDMSOA, colsNowA=colsNow, compClass="Class1")
  }, height=250, width=250)
  # Histograms for second class of compound ----
  output$class2pdf <- renderPlot({
    # Read in needed values
    SSMD.NowA=as.numeric(input$SSMD.t2)
    sd.NowA=input$sd.t2
    metricNowA=input$metricNow
    dfPerFOVA=dfCloseToNormal - input$tailWeight
    mu.DMSOA=input$mu.DMSO
    sdDMSOA=input$sdDMSO
    getGraphCentered(SSMD.Now=SSMD.NowA, sd.Now=sd.NowA, metricNow=metricNowA,
      dfPerFOV=dfPerFOVA, mu.DMSO=mu.DMSOA, sdDMSO=sdDMSOA, colsNowA=colsNow, compClass="Class2")
  }, height=250, width=250)
  # Histograms for third class of compound ----
  output$class3pdf <- renderPlot({
    # Read in needed values
    SSMD.NowA=as.numeric(input$SSMD.t3)
    sd.NowA=input$sd.t3
    metricNowA=input$metricNow
    dfPerFOVA=dfCloseToNormal - input$tailWeight
    mu.DMSOA=input$mu.DMSO
    sdDMSOA=input$sdDMSO
    getGraphCentered(SSMD.Now=SSMD.NowA, sd.Now=sd.NowA, metricNow=metricNowA,
      dfPerFOV=dfPerFOVA, mu.DMSO=mu.DMSOA, sdDMSO=sdDMSOA, colsNowA=colsNow, compClass="Class3")
  }, height=250, width=250)
