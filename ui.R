...
# Define UI 
ui <- fluidPage(
  title="Aid for planning SPT screening experiments",
  # Intro / explanations ----
  fluidRow(
    column(12, 
      h2("Aid for planning SPT screening experiments."),
  ...
  fluidRow(
    column(4, sliderInput("nCompounds", "Total number of non-control compounds", 
    	min = 1e4, max = 2e5, value=1.5e5, step=2e3),
    	sliderInput("nFOVsPerWell", "Number FOVs (number of data points) per well", 
    ...
    column(4, selectInput("metricNow", "Choose a metric:",
    	...
    	selectInput("pi0", HTML(paste("Prop'n of non-active compounds, ie, &pi;<sub>0</sub>",
    	"(Prop'n of compounds like neg control)")),
    	c("0.9" =0.9, '0.99'=0.99, "0.999" =0.999, '0.9999'=0.9999), selected="0.99")),
    column(4, # h4(HTML(paste("<font color='red'><b>How heavy should the tail be?</b></font>"))),
      h4(HTML(paste("How heavy should the tail be?"))),
      sliderInput("tailWeight", HTML(paste("Lower is more like a normal distribution")), 
      min = 1, max = 38.9, value=36, step=0.1)),
      HTML("<div style='height: 250px;'>"), # Hacky fix for image height https://stackoverflow.com/questions/30800413/shiny-r-unwanted-white-space-in-fluidrow
